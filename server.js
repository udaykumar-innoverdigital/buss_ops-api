...
// New API endpoint to fetch ClientID and client name
...
// New API endpoint to fetch project details for a specific client
...
// API endpoint to get allocation snapshot for a specific date
app.get('/api/allocation-snapshot', (req, res) => {
  ...
  const query = `
    SELECT
      SUM(CASE 
        WHEN a.AllocationPercent = 100 AND a.ClientID != 1 THEN 1 
        ELSE 0 
      END) AS allocated_count,
      SUM(CASE 
        WHEN a.AllocationID IS NULL THEN 1 
        ELSE 0 
      END) AS unallocated_count,
      SUM(CASE 
        WHEN a.AllocationPercent > 0 AND a.AllocationPercent < 100 THEN 1 
        ELSE 0 
      END) AS draft_count,
      SUM(CASE 
        WHEN a.ClientID = 1 AND a.AllocationPercent > 0 THEN 1 
        ELSE 0 
      END) AS bench_count
    FROM 
      Employees e
    LEFT JOIN 
      Allocations a ON e.EmployeeId = a.EmployeeID
        AND ? BETWEEN a.AllocationStartDate AND IFNULL(a.AllocationEndDate, '9999-12-31')
    WHERE 
      e.EmployeeKekaStatus = 'Active'
      AND (e.EmployeeJoiningDate <= ? OR e.EmployeeJoiningDate IS NULL)
      AND (e.EmployeeEndingDate > ? OR e.EmployeeEndingDate IS NULL)
  `;
  ...
  db.query(query, [date, date, date], (err, results) => {
    ...
    res.json({
      ...
      unallocated: snapshot.unallocated_count,
      draft: snapshot.draft_count,
      bench: snapshot.bench_count
    });
  });
});
...
app.post('/api/allocate', (req, res) => {
  ...
  // Fetch current and staged allocations within the date range
  const allocationQuery = `
    SELECT 
      IFNULL(SUM(CASE WHEN AllocationStatus IN ('Active', 'Allocated') THEN AllocationPercent ELSE 0 END), 0) AS totalAllocation,
      IFNULL(SUM(CASE WHEN AllocationStatus = 'Staged' THEN AllocationPercent ELSE 0 END), 0) AS stagedAdditions
    FROM Allocations
    WHERE 
      EmployeeID = ? AND 
      (
  ...
  db.query(allocationQuery, [EmployeeID, AllocationEndDate || '9999-12-31', AllocationStartDate], (err, results) => {
    if (err) {
      console.error('Error fetching allocations:', err);
      return res.status(500).json({ message: 'Internal Server Error' });
    }

    const { totalAllocation, stagedAdditions } = results[0];
    const newTotal = totalAllocation + stagedAdditions + AllocationPercent;
    ...
    db.query(overlapQuery, [EmployeeID, ProjectID, AllocationEndDate || '9999-12-31', AllocationStartDate], (err, overlapResults) => {
      if (err) {
        console.error('Error checking overlapping allocations:', err);
        return res.status(500).json({ message: 'Internal Server Error' });
      }
